...
class JobDispatcher:
    ...
    def updateJob(
        ...
        if jobStatus == "failed" or jobStatus == "finished":
            ...
            param["jobDispatcherErrorDiag"] = f"set to {jobStatus} by the pilot at {datetime.datetime.now(datetime.timezone.utc).strftime('%Y-%m-%d %H:%M:%S')}"
...
def getJob(
    ...
    tmpLog = LogWrapper(_logger, f"getJob {datetime.datetime.now(datetime.timezone.utc).isoformat('/')}")
