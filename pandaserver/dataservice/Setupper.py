                    ...
                    if status != 0 or out.find('Error') != -1:
                        ...
                        if srcDQ2ID == dstDQ2ID:
                            ...
                            if job.cloud != self.siteMapper.getSite(job.computingSite).cloud and \
                                   ...
                                   (not job.prodSourceLabel in ['user','panda']) and \
                                   not self.siteMapper.getSite(job.computingSite).cloud in ['ND','ES','DE','NL','TW','CERN']:
                                seTokens = self.siteMapper.getSite(job.computingSite).setokens
                                # use T1_PRODDISK
    ...
    def _makeDisDatasetsForExistingfiles(self):
        ...
        for tmpJob in self.jobs:
            ...
            if tmpJob.cloud != self.siteMapper.getSite(tmpJob.computingSite).cloud and \
               ...
               not self.siteMapper.getSite(tmpJob.computingSite).cloud in ['ND','ES','DE','NL','TW','CERN']:
