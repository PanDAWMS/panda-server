...
def main(argv=tuple(), tbuf=None, **kwargs):
    ...
    try:
        timeLimit = datetime.datetime.now(datetime.UTC) - datetime.timedelta(hours=2)
        # get PandaIDs
        varMap = {}
    ...
    timeLimit = datetime.datetime.now(datetime.UTC) - datetime.timedelta(hours=workflow_timeout_map["analysis"])
    varMap = {}
    ...
    timeLimit = datetime.datetime.now(datetime.UTC) - datetime.timedelta(minutes=30)
    varMap = {}
    ...
    timeLimit = datetime.datetime.now(datetime.UTC) - datetime.timedelta(hours=3)
    # get XMLs
    xmlIDs = set()
    ...
    timeOutVal = 3
    timeLimit = datetime.datetime.now(datetime.UTC) - datetime.timedelta(hours=timeOutVal)
    ...
    if not timeOutVal:
        timeOutVal = 48 * 60
    timeLimit = datetime.datetime.now(datetime.UTC) - datetime.timedelta(minutes=timeOutVal)
    ...
    timeLimit = datetime.datetime.now(datetime.UTC) - datetime.timedelta(days=7)
    status, res = taskBuffer.querySQLS(
    ...
    timeLimitSite = datetime.datetime.now(datetime.UTC) - datetime.timedelta(hours=inactiveTimeLimitSite)
    timeLimitJob = datetime.datetime.now(datetime.UTC) - datetime.timedelta(hours=inactiveTimeLimitJob)
    # get PandaIDs
    ...
    if not timeoutValue:
        timeoutValue = 4 * 60
    timeLimit = datetime.datetime.now(datetime.UTC) - datetime.timedelta(minutes=timeoutValue)
    # get PandaIDs
    ...
    timeLimit = datetime.datetime.now(datetime.UTC) - datetime.timedelta(hours=12)
    ...
    timeLimit = datetime.datetime.now(datetime.UTC) - datetime.timedelta(days=2)
    status, res = taskBuffer.lockJobsForReassign(
    ...
    timeLimit = datetime.datetime.now(datetime.UTC) - datetime.timedelta(hours=48)
    status, res = taskBuffer.lockJobsForReassign(
        "ATLAS_PANDA.jobsActive4",
        timeLimit,
        ["starting"],
    ...
    timeLimit = datetime.datetime.now(datetime.UTC) - datetime.timedelta(hours=24)
    varMap = {}
    varMap[":jobStatus1"] = "running"
    varMap[":jobStatus2"] = "starting"
    varMap[":timeLimit"] = timeLimit
    ...
    try:
        normalTimeLimit = datetime.datetime.now(datetime.UTC) - datetime.timedelta(hours=timeoutVal)
        sortTimeLimit = datetime.datetime.now(datetime.UTC) - datetime.timedelta(hours=3)
        sql = (
            "WITH p AS ("
        ...
        timeoutMap = {}
        if resList != []:
            recentRuntimeLimit = datetime.datetime.now(datetime.UTC) - datetime.timedelta(hours=3)
                ...
                if tmpSiteStatus not in ["offline", "test"]:
                    if workingGroup:
                        if workingGroup not in timeoutMap:
                            ...
                            if tmp_timeoutVal:
                                timeoutMap[workingGroup] = datetime.datetime.now(datetime.UTC) - datetime.timedelta(hours=tmp_timeoutVal)
                            else:
    ...
    timeLimit = datetime.datetime.now(datetime.UTC) - datetime.timedelta(days=21)
    status, res = taskBuffer.querySQLS(
    ...
    timeLimit = datetime.datetime.now(datetime.UTC) - datetime.timedelta(days=5)
    varMap = {}
    ...
    timeLimit = datetime.datetime.now(datetime.UTC) - datetime.timedelta(days=1)
    sqlC = (
    ...
    timeLimit = datetime.datetime.now(datetime.UTC) - datetime.timedelta(days=1)
    expireLimit = datetime.datetime.now(datetime.UTC) - datetime.timedelta(days=30)
