...
def main(argv=tuple(), tbuf=None, **kwargs):
    ...
    # current minute
    current_minute = datetime.datetime.now(datetime.UTC).minute
    ...
    try:
        ...
        if (timeNow > logRotateTime and (timeNow - logRotateTime) < datetime.timedelta(minutes=5)) or (
            ...
            timeLimitS = datetime.datetime.now(datetime.UTC) - datetime.timedelta(hours=1)
            ...
            # tmp name
            tmp_logName = f"{dispLogName}.tmp-{datetime.datetime.now(datetime.UTC).strftime('%Y-%m-%d-%H-%M-%S')}"
            # loop over all files
            pilotCounts = {}
            pilotCountsS = {}
