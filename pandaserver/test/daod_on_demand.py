...
s,o = proxy.getClobObj('select JEDI_TASK_PARAMETERS from atlas_deft.t_task where taskid=:task_id',{':task_id':jediTaskID})

taskParamStr = o[0][0]

t = json.loads(taskParamStr)

s,o = proxy.querySQLS('select cpuTime,cpuTimeUnit,ioIntensity,ioIntensityUnit,ramCount,ramUnit,outDiskCount,outDiskUnit,workDiskCount,workDiskUnit '
                      'FROM atlas_panda.jedi_tasks where jeditaskid=:task_id',{':task_id':jediTaskID})
cpuTime,cpuTimeUnit,ioIntensity,ioIntensityUnit,ramCount,ramUnit,outDiskCount,outDiskUnit,workDiskCount,workDiskUnit = o[0]
