...
# instantiate TB
requester_id = "{0}({1})".format(__name__, GenericThread().get_pid())
...
for file in job.Files:
    if file.type in ['output']:
        ...
        meta += """{ "AODFixVersion": "", "beam_energy": [ 6500000.0 ], "beam_type": [ "collisions" ], "conditions_tag": "OFLCOND-RUN12-SDR-14", "file_guid": "%s", "file_size": 643794695, "file_type": "pool", "geometry": "ATLAS-R2-2015-01-01-00", "integrity": true, "lumi_block": [ 15 ], "name": "%s", "run_number": [ 222222 ] } ,""" % (
            file.GUID, file.lfn)
meta = meta[:-1]
