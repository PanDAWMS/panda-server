...
def insertTaskParams(req, taskParams=None, properErrorCode=None, parent_tid=None):
    tmpLog = LogWrapper(_logger, f"insertTaskParams-{datetime.datetime.now(datetime.timezone.utc).isoformat('/')}")
    tmpLog.debug("start")
...
def updateWorkers(req, harvesterID, workers):
    ...
    host = req.get_remote_host()
    retVal = None
    tStart = datetime.datetime.now(datetime.timezone.utc)
    # convert
    try:
    ...
    tDelta = datetime.datetime.now(datetime.timezone.utc) - tStart
...
def relay_idds_command(req, command_name, args=None, kwargs=None, manager=None, json_outputs=None):
    tmpLog = LogWrapper(
        _logger,
        f"relay_idds_command-{datetime.datetime.now(datetime.timezone.utc).isoformat('/')}",
    )
    # check security
    if not isSecure(req):
...
def execute_idds_workflow_command(req, command_name, kwargs=None, json_outputs=None):
    try:
        tmpLog = LogWrapper(
            _logger,
            f"execute_idds_workflow_command-{datetime.datetime.now(datetime.timezone.utc).isoformat('/')}",
        )
        if kwargs:
            try:
                kwargs = idds.common.utils.json_loads(kwargs)
...
def set_user_secret(req, key=None, value=None):
    tmpLog = LogWrapper(_logger, f"set_user_secret-{datetime.datetime.now(datetime.timezone.utc).isoformat('/')}")
    # get owner
    dn = req.subprocess_env.get("SSL_CLIENT_S_DN")
    if not dn:
        tmpMsg = "SSL_CLIENT_S_DN is missing in HTTP request"
...
def get_user_secrets(req, keys=None, get_json=None):
    tmpLog = LogWrapper(_logger, f"get_user_secrets-{datetime.datetime.now(datetime.timezone.utc).isoformat('/')}")
    # get owner
    dn = req.subprocess_env.get("SSL_CLIENT_S_DN")
    if get_json == "True":
        get_json = True
