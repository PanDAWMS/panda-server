...
def sendMsgToLoggerHTTP(msgList, job):
    try:
        ...
        if job.jobsetID not in [None, "NULL"]:
            msgHead = "dn='%s' : jobset=%s jobdef=%s" % (
                job.prodUserName,
                job.jobsetID,
                job.jobDefinitionID,
            )
        ...
        for job in jobs + [None]:
            ...
            if (
                            ...
                            if prevIsJEDI:
                                ...
                                if specialWeight != {}:
                                    ...
                                    if site in sizeMapForCheck:
                                        ...
                                        tmpLog.debug(
                                            "   space available=%s remain=%s thr=%s"
                                            % (
                                                sizeMapForCheck[site]["total"],
                                                remSpace,
                                                thrForThisSite,
                                            )
                                        )
                                ...
                                if specialWeight != {}:
                                    ...
                                    if _isTooManyInput(nFilesPerJob, inputSizePerJob):
                                        if site == siteMapper.getCloud(previousCloud)["source"] or (
                                            ...
                                            winv /= cloudT1Weight
                                            tmpLog.debug(
                                                "   special weight for %s : nInputs/Job=%s inputSize/Job=%s weight=%s"
                                                % (
                                                    site,
                                                    nFilesPerJob,
                                                    inputSizePerJob,
                                ...
                                for tmpItem in resultsForAnal["maxtime"]:
                                    if siteMapper.checkSite(tmpItem):
                                        resultsForAnalStr += "%s:%s," % (
                                            tmpItem,
                                            siteMapper.getSite(tmpItem).maxtime,
                                        )
                ...
                if job.prodDBlock != "NULL":
                    ...
                    dispatchDBlock = "panda.%s.%s.%s.%s.%s_dis%s" % (
                        job.taskID,
                        time.strftime("%m.%d"),
                        tmpDataType,
                        transferType,
