...
class Proxy(WorkflowModule):
    def __init__(s, task_id=49900001, active_count=0, max_tasks=None):
        s.backend = "oracle"
        # The module under test reads self.cur, which the real BaseModule declares as a
        # WrappedCursor. fake_cur is the same object under a name the checks can read `executed`
        # from without claiming the stand-in is one.
        s.fake_cur = Cur(task_id, active_count)
        s.cur = cast(Any, s.fake_cur)
        s._max = max_tasks
    ...
    def _rollback(s, useOtherError=False):
        return True
...
sqls = [e[0] for e in p.fake_cur.executed]
...
ins = [e for e in p.fake_cur.executed if "INSERT INTO ATLAS_DEFT.T_TASK" in e[0]][0]
...
upd = [e for e in p.fake_cur.executed if "UPDATE" in e[0]][0]
...
ok("no UPDATE issued", not any("UPDATE" in q for q, _ in p2.fake_cur.executed))

print("\nexplicit parent_tid:")
p3 = Proxy()
p3.insert_step_task(dict(base), "/DC=ch/CN=t", parent_tid=48810693)
ins3 = [e for e in p3.fake_cur.executed if "INSERT INTO" in e[0]][0]
...
ok("nothing inserted", not any("INSERT INTO ATLAS_DEFT.T_TASK" in q for q, _ in p4.fake_cur.executed))
...
ok("no limit configured -> no throttle query", tid6 == 49900001 and not any("COUNT(*)" in q for q, _ in p6.fake_cur.executed))
...
p7 = MyProxy()
p7.fake_cur = MyCur(49900001, 0)
p7.cur = cast(Any, p7.fake_cur)
tid7, msg7 = p7.insert_step_task(dict(base), "/DC=ch/CN=t")
ok("inserts via the stand-in sequence table", tid7 == 49900001, msg7)
q7 = [e for e in p7.fake_cur.executed if "INSERT INTO ATLAS_DEFT.T_TASK" in e[0]][0]
