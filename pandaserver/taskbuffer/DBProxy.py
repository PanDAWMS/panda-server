...
class DBProxy:
    ...
    def insertNewJob(self,job,user,serNum,weight=0.0):
        ...
        if job.prodSourceLabel == 'user':
            job.currentPriority = 1000 - (serNum / 5) - int(100 * weight)
        # usergroup
            ...
            try:
                ...
                if (((job.prodSourceLabel == 'user' or job.prodSourceLabel == 'panda') \
                    ...
                    if job.computingSite.startswith('ANALY') and (not job.computingSite.startswith('ANALY_LONG_')):
                        job.computingSite = re.sub('^ANALY_','ANALY_LONG_',job.computingSite)
                        job.computingSite = re.sub('_\d+$','',job.computingSite)
    ...
    def getJobs(self,nJobs,siteName,prodSourceLabel,cpu,mem,diskSpace,node,timeout,computingElement,
        ...
        if prodSourceLabel == 'user':
            ...
            dynamicBrokering = True
            sql1+= "AND (computingElement='%s' OR computingElement='to.be.set') " % computingElement
        else:
        ...
        try:
            ...
            for iJob in range(nJobs):
                ...
                for resF in resFs:
                    ...
                    job.addFile(file)
                # dynamic brokering
                if dynamicBrokering or job.computingElement=="to.be.set":
                    # check computingElement
                    if job.computingElement == "to.be.set":
                        sqlE = "UPDATE jobsActive4 SET computingElement='%s' " % computingElement
                        sqlE+= 'WHERE PandaID=%s'
                        self.cur.execute((sqlE+comment) % pandaID)
                    else:
                        dynamicBrokering = False
                ...
                if dynamicBrokering:
                    # look for downstream jobs
                    sqlD   = "SELECT PandaID FROM filesTable4 WHERE type='input' AND lfn='%s' GROUP BY PandaID"
                    sqlDJD = "UPDATE jobsDefined4 SET computingElement='%s' " % computingElement
                    sqlDJD+= "WHERE PandaID=%s" 
                    ids = []
                    for upFile in job.Files:
                        _logger.debug("look for downstream jobs for %s" % upFile.lfn)
                        # select PandaID
                        self.cur.execute((sqlD+comment) % upFile.lfn)
                        res = self.cur.fetchall()
                        for downID in res:
                            if not downID in ids:
                                ids.append(downID)
                                _logger.debug("change CE : %s" % downID)        
                                # update CE
                                self.cur.execute((sqlDJD+comment) % downID)
                # append
                retJobs.append(job)
            # commit
            if not self._commit():
    ...
    # kill job
    def killJob(self,pandaID,user,code):
        comment = ' /* DBProxy.killJob */'        
        _logger.debug("killJob : %s %s" % (code,pandaID))
        # check PandaID
        try:
            long(pandaID)
        except:
        ...
        try:
            ...
            for table in ('jobsDefined4','jobsActive4','jobsWaiting4'):
                ...
                if (not user.startswith('/DC=org/DC=doegrids/OU=People/CN=Nurcan Ozturk')) \
    ...
    def queryLastFilesInDataset(self,datasets):
        ...
        if len(datasets) == 0:
            return []
        # make SQL query
        sql1 = "SELECT MAX(PandaID) FROM filesTable4 WHERE dataset=%s"
