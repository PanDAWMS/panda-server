...
class DBProxy:
    ...
    def getTaskParamsMap(self, jediTaskID):
        """
        Will return taskParamsMap with jediTaskID
        @param jediTaskID: jedi task id
        """

        comment = ' /* DBProxy.getTaskParamsMap */'
        method_name = comment.split(' ')[-2].split('.')[-1]
        tmp_log = LogWrapper(_logger, method_name)
        tmp_log.debug('start')
        ...
        try:
            ...
            varMap[':jediTaskID'] = jediTaskID
            sql_taskparams = """
                   SELECT taskparams  FROM  {0}.JEDI_TASKPARAMS
                   WHERE jediTaskID=:jediTaskID
                   """.format(panda_config.schemaJEDI)

            self.cur.execute(sql_taskparams + comment, varMap)
            res = self.cur.fetchone()
