...
class JobStandaloneModule(BaseModule):
    ...
    # get job statistics with prodsourcelabel
    def getJobStatisticsWithProdsourcelabel(self):
        comment = " /* DBProxy.getJobStatisticsWithProdsourcelabel */"
        tmp_log = self.create_tagged_logger(comment)
        tmp_log.debug("start")
        ...
        for retry in range(max_retries):
            try:
                for table in tables:
                    ...
                    # create map
                    for computing_site, prodsourcelabel, job_status, n_jobs in res:
                        if job_status in excluded_states:  # ignore some job status since they break APF
                            continue

                        ret.setdefault(computing_site, {}).setdefault(prodsourcelabel, {}).setdefault(job_status, 0)
                        ret[computing_site][prodsourcelabel][job_status] += n_jobs

                # fill in missing states with 0
                for site in ret:
                    for prodsourcelabel in ret[site]:
                        for state in included_states:
                            ret[site][prodsourcelabel].setdefault(state, 0)
